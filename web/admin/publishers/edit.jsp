<%-- 
    Document   : edit
    Created on : Nov 13, 2024, 12:41:21 AM
    Author     : VIET
--%>
...
<html>
    ...
    <form action="${p.getPublisherID()}" method="POST" enctype="multipart/form-data" class="border shadow p-5 rounded-md max-w-screen-md mx-auto space-y-3">
        ...
        <div class="space-y-3">
            <div>
                ...
                <input name="publisher-name" required="true" placeholder="Publisher Name" 
                       value="${p.getPublisherName()}"
                       class="w-full border-2 px-2 py-1 outline-none rounded-md border-black/60"/>
            </div>

            <div>
                <label class=" block pb-2">Publisher Address </label>
                <input name="publisher-address" value="${p.getAddress()}" required="true" placeholder="Publisher Address" 
                       class="w-full border-2 px-2 py-1 outline-none rounded-md border-black/60"/>
            </div>

            <div>
                <label class=" block pb-2">Publisher Email </label>
                <input name="publisher-email" value="${p.getEmail()}" required="true" placeholder="Publisher Email" 
                       class="w-full border-2 px-2 py-1 outline-none rounded-md border-black/60"/>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="text" required="true" placeholder="1990" 
                           name="publisher-establishYear" value="${p.getEstablishYear()}"
                           class="w-full border-2 px-2 py-1 outline-none rounded-md border-black/60"/>
                </div>
                <div>
                    <label class="block pb-2">Image</label>
                    <div class="flex">
                        <img class="w-20 h-20 rounded-md mr-2" id="preview" src="${p.getImage()}" alt="alt"/>
                        <input name="image" type="text" value="${p.getImage()}" hidden/>
            ...
            <div>
                ...
                <textarea name="publisher-description" required="true" rows="6"
                          class="w-full border-2 px-2 py-1 outline-none rounded-md border-black/60" 
                          placeholder="Write somethings about this publisher">${p.getDescription()}</textarea>
            </div>
